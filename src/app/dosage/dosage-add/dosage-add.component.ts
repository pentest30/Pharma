...
@Component({
  selector: 'app-dosage-add',
  templateUrl: './dosage-add.component.html',
  styleUrls: ['./dosage-add.component.sass']
})
export class DosageAddComponent implements OnInit {
  ...
  constructor(public snackBar: MatSnackBar, private fb: FormBuilder,
    private dialogRef: MatDialogRef<DosageAddComponent>,
    @Inject(MAT_DIALOG_DATA) data,
    private service: DosageService, private notifHelper :NotificationHelper) {
      this.id = data.id;
      this.name = data.name;
      if(data.id == null)
